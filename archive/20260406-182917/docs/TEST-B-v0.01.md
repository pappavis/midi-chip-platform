...
## Appendix K — MIDI note reference (for consistent testing)

When writing observations, it helps to use consistent note names.

- Middle C is often called C4, but some systems call it C3.
- The test plan uses observable outcomes (“pitch goes up/down”) rather than absolute note naming.

Practical testing guidance:
- Use single notes first.
- Then test a simple chord.
- Then test rapid repeats.

This reduces confusion when comparing results between DAWs/controllers.
