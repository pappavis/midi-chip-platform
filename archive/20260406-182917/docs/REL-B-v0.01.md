...
## 29. Link integrity and doc polish checklist

Before publishing, do a quick “docs polish” pass:

- [ ] All relative links in `docs/` resolve.
- [ ] Code fences are correctly closed.
- [ ] Port names in examples match `midi list` output (or are clearly labeled as examples).
- [ ] Every doc repeats the validation boundary (macOS confirmed; Pi pending).

If you automate this later, great—but even a manual 2-minute scan prevents embarrassing drift.

---

## 30. Release note diff checklist (avoid regressions in messaging)

When preparing release notes, compare to the previous release notes and ask:

- Did we accidentally broaden the support statement?
  - e.g., changing “validated on macOS” to “works on macOS” without a ledger update.

- Did we remove the “pending” label for Pi?
  - Only remove it when a ledger entry exists.

- Did we add performance language?
  - If yes, ensure there is measurement and evidence.

This prevents accidental scope creep in communication.
