...
## Appendix S — Debug bundle (what to collect when asking for help)

If you need support (or you are writing an issue), collect this “debug bundle”:
...
- `midi list` output
- 5–10 lines of `monitor` output (if routing involved)

This bundle usually makes the root cause obvious.

---

## Appendix T — Troubleshooting philosophy reminder

When something fails, resist the temptation to change multiple variables.

Change one thing at a time:
- confirm audio (`test basic`)
- confirm routing (`monitor`)
- then confirm end-to-end (`run`)

This is the fastest path to a confident fix.

---

## Appendix U — Command-line hygiene (tiny details that prevent failure)

- Always activate the venv before running commands.
- Always quote port names.
- Prefer explicit flags (`--midi-port`, `--midi-channel`) for demos.
- When copying commands from docs, paste them exactly first; only then customize.

These habits reduce “it worked yesterday” surprises.

---

## Appendix V — Tiny FAQ (the questions that waste the most time)

### “Do I have to use channel 3?”

No. Channel 3 is the documented demo convention. You can use another channel if both sender (Logic/controller) and receiver (`--midi-channel`) match.

### “Why do I need quotes around the port name?”

Because port names include spaces and sometimes special characters.
Without quotes, the shell splits the name and the CLI receives a broken value.

### “Why does `monitor` matter?”

Because it is the fastest proof of routing. If `monitor` shows nothing, the synth is not the problem yet.
